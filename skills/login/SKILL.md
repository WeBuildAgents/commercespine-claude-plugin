---
description: Sign in to Commerce Spine and store an API token for the plugin. Use when the user runs /commercespine:login, asks to connect/authenticate/sign in to Commerce Spine, or when another Commerce Spine skill reports a missing or rejected token.
disable-model-invocation: true
---

# Connect to Commerce Spine

Run the login command to authenticate the user and store their Commerce Spine API token.
...
2. This opens the user's browser to the Commerce Spine sign-in / token page. The user
   ...
   let the user select their existing tokens. Commerce Spine cannot re-reveal a token
   secret after it is created, so there is nothing to pick from. Every
   `/commercespine:login` therefore issues an additional token on the user's account.
   ...
   Commerce Spine web app to review or revoke tokens.
...
3. Report the outcome to the user based on the command's exit status:
   - **Exit 0** — connected and verified. Tell the user they can now ask data
     questions (the `commercespine:data-layer` skill will answer them). Then go to
     step 4.
   - **Exit 2** — the token was saved but Commerce Spine **rejected** it. Signing in
     again is the right fix: tell the user to run `/commercespine:login` once more,
...
4. **Only on Exit 0**, ask the user whether they'd like to see the Amazon
   ...
   - If they say **yes**, hand off to the `commercespine:data-layer` skill to run the
...
- The token is stored at `~/.config/commercespine/credentials.json` with `0600`
  permissions. Never print the token.
- To check connection status without re-authenticating, run `commercespine-config`.
